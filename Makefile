...
reload_run:
	git ls-files --cached --others | grep -v '_test.go$$' | grep '.go$$' | entr -c -r go run . -addr :5555 -stats

reload_run_db:
	git ls-files --cached --others | grep -v '_test.go$$' | grep '.go$$' | entr -c -r go run . -addr :5556 -db cache.db -stats
